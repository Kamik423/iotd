...
POST_CACHE_COUNT = 16
CACHE = SCIRPT_DIR / "red_panda_cache.txt"
...
def post_of_the_day(reddit: praw.Reddit, subreddit="redpandas") -> tuple[str, str]:
    ...
    for chat_id in chat_ids:
        try:
            bot.send_photo(
                chat_id=chat_id,
                photo=image,
                caption=f"Red Panda of the day 🤗 ({today}) "
                f'<a href="{post_url}">🌐</a>',
